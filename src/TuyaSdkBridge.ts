...
export class TuyaSdkBridge {
  ...
  private static readonly searchingGwDeviceEventName =
    'kNotificationFindGatewayDevice';
  private static readonly searchingSubDeviceEventName =
    'kNotificationResultSubDevice';
  ...
  private static wiredGwSearchingEventFunctionPointer: (
    gw_id: string,
    product_id: string
  ) => void;
  ...
  public static startSearchWiredGW(
  ...
  public static stopSearchWiredGW() {
    ...
    if (TuyaSdkBridge.subscriptionForGw != null) {
      ...
      TuyaSdkBridge.debugLogEventFunctionPointer(
        debugCode.WARN_START_SEARCH_WIREDGW_FIRST
      );
  ...
  public static async registerWiredGW(
    gw_id: string,
    product_id: string,
    timeout: number
  ): Promise<any> {
    ...
    if (TuyaSdkBridge.initialized == false) {
      ...
      TuyaSdkBridge.debugLogEventFunctionPointer(
        debugCode.ERR_REGISTER_WIREDGW_ALREADY
      );
    } else {
      ...
      await TuyaNative.initSearchedGwDevice(passParam).then(
        (okRes: any) => {
          returnValue = okRes;
          const CombinationName: string = TuyaSdkBridge.getCombinationTuyaName(
            okRes.name
          );
          TuyaNative.renameDevice({
            devId: okRes.devId,
  ...
  public static async startRegisterZigbeeSubDevice(
    ...
    if (TuyaSdkBridge.initialized == false) {
      ...
      TuyaSdkBridge.debugLogEventFunctionPointer(
        debugCode.ERR_START_REGISTER_ZIGBEE_SUBDEVICE_ALREAY
      );
    } else {
  ...
  public static async stopRegisterZigbeeSubDevice() {
    ...
    if (TuyaSdkBridge.subscriptionForSubDevice) {
      if (Platform.OS === 'ios') {
        TuyaNative.stopNewGwSubDevActivatorConfig({
          devId: TuyaSdkBridge.targetGwIdForSubDevice,
        });
      } else {
      ...
      TuyaSdkBridge.debugLogEventFunctionPointer(
        debugCode.WARN_START_REGISTER_ZIGBEE_SUBDEVICE_FIRST
      );
    }
  ...
  public static async removeDevice(
    devId: TuyaNative.RemoveDeviceParams,
    reset: boolean
  ): Promise<boolean> {
  ...
  private static async subDeviceEventInternalFunction(result: any) {
    ...
    if (result.result == 'onError') {
      ...
      try {
        await TuyaNative.renameDevice({
          devId: result.var1.devId,
          name: combinationName,
        });
  ...
  private static getCombinationTuyaName(deviceName: string): string {
    ...
    if (
      postNameValue.length + deviceName.length + 1 >
      TuyaSdkBridge.CombinationNameMaxLength
    ) {
      deviceName = deviceName.substring(
        0,
        TuyaSdkBridge.CombinationNameMaxLength - postNameValue.length - 1
      );
    }
  ...
  private static async searchingInternalFunction(
    gwId: string,
    productId: string
  ) {
    if (TuyaSdkBridge.subscriptionForGw != null) {
  ...
  private static async searchingInternalFunctionForAndroid(
    gwInfo: TuyaNative.HgwBean
  ) {
  ...
  private static async searchingInternalFunctionForIos(gwInfo: {
    gwId: string;
    productId: string;
  }) {
    // Android has more information, pick some to fit common interface
    ...
    await TuyaNative.getCurrentUser()
      ...
      .catch(async error => {
        ...
        await TuyaNative.touristRegisterAndLogin({
          ...
          .then(async () => {
            TuyaSdkBridge.debugLogEventFunctionPointer(
              debugCode.INF_CREATE_ANONYMOUS_ACCOUNT
            );
            await TuyaNative.getCurrentUser()
              .then(async (result: TuyaNative.User | null) => {
                const { uid } = result!;
                await this.userLoginFunction(uid).then(
                  OkRes => {
                    console.log('this.userLoginFunction:', OkRes);
                    returnValue = true;
                  },
                  async NgRes => {
                    console.log('this.userLoginFunction:', NgRes);
                    await this.logout();
                  }
                );
              })
              .catch(error => {
                ...
                TuyaSdkBridge.debugLogEventFunctionPointer(
                  debugCode.ERR_GET_CURRENT_USER
                );
              });
          })
          .catch(error => {
            ...
            TuyaSdkBridge.debugLogEventFunctionPointer(
              debugCode.ERR_CREATE_ANONYMOUS_ACCOUNT
            );
          });
      });
